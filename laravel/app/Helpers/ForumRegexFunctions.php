...
/** Extract quoted text. */
function quoteSubstitution(string $str): string
{
  /** 
...
function extractCommentsReply(string $str): string
{
  /**
   * (#1) Remove line breaks from text coming from database. 
   * Must be used otherwise regex won't work properly. 
...
function removeBreaksBetweenQuotes(string $str): string
{
  $pattern = [
    "%^<p><br></p>%",
    "%(?s)<p>\[/QUOTE\]</p><p><br></p>%"
...
function removeNewlinesAndSingleBreak(string $str): string
{
  /**
...
function stringLengthWithoutSpaces(string $str): int
{
